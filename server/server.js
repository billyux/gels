...
app.post("/api/auth/signup", (req, res) => {
  ...
  const existing = db.prepare("SELECT id FROM users WHERE username = ?").get(cleanedUsername);
  if (existing) {
  ...
  const info = db
    ...
    .run(cleanedUsername, passwordHash, name);
...
app.post("/api/auth/signin", (req, res) => {
  ...
  const row = db.prepare("SELECT * FROM users WHERE username = ?").get(cleanedUsername);
...
app.get("/api/attendance", requireAuth, (req, res) => {
  ...
  const rows = db
    ...
    .all(req.user.id, start || "0000-01-01", end || "9999-12-31");
  res.json({ rows });
});
...
app.post("/api/posts", requireAuth, upload.single("image"), (req, res) => {
  ...
  const info = db
    .prepare("INSERT INTO posts (user_id, title, content, image_url) VALUES (?, ?, ?, ?)")
    .run(req.user.id, title, content, imageUrl);
...
/* ======================
   물 섭취 기록
====================== */
app.get("/api/water-log/today", requireAuth, (req, res) => {
  const today = new Date().toISOString().slice(0, 10);
  const row = db
    .prepare("SELECT cups FROM water_log WHERE user_id = ? AND logged_on = ?")
    .get(req.user.id, today);
  res.json({ cups: row?.cups || 0 });
});

app.post("/api/water-log/increment", requireAuth, (req, res) => {
...
app.get("/api/water-log", requireAuth, (req, res) => {
  const { start, end } = req.query;
  const rows = db
    .prepare(
...
/* ======================
   활동량 기록 (하드웨어 로거)
====================== */
app.post("/api/activity-samples", requireAuth, (req, res) => {
  ...
  db.prepare(
    "INSERT INTO activity_samples (user_id, activity_level, posture) VALUES (?, ?, ?)"
  ).run(req.user.id, activity_level, posture || null);
...
app.get("/api/shelters", requireAuth, async (req, res) => {
